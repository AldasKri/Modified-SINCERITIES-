...
#Kullback-Leiber 
        ...
        p1[p1 == 0] <- 1e-10
        p2[p2 == 0] <- 1e-10
        DISTANCE_matrix[ti,gi] <- sum(p1 * log(p1 / p2))
      }
#Jensen-Shannon
      else if(distance==6){
        p1[p1 == 0] <- 1e-10
        p2[p2 == 0] <- 1e-10
        m <- 0.5 * (p1+ p2)
        DISTANCE_matrix[ti,gi] <- ((1/2)*(sum(p1 * log(p1 / m))) + (1/2)*(sum(p2 * log(p2 / m))))
      }
#Hellinger Distance
      else if(distance==7){
        DISTANCE_matrix[ti,gi] <- sqrt(0.5 * sum((sqrt(p1) - sqrt(p2))^2))
      }
